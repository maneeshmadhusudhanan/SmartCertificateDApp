...
describe("Pass Test", function () {
  ...
  it("Testing issue", async function () {
    ...
    await expect(cert.issue(101, "Maneesh", "EDP", "M", "30-12-2024"))
      .to.emit(cert, "Issued")
      .withArgs("EDP", 101, "S");
  });
  ...
  it("Read value", async function () {
    const { cert } = await loadFixture(deployCertFixture);
    await cert.issue(101, "Maneesh", "EDP", "M", "30-12-2024");
    const certificates = await cert.Certificates(101);

    // console.log(certificates);

    expect(certificates[0]).equal("Maneesh");
    expect(certificates[1]).equal("EDP");
    expect(certificates[2]).equal("M");
    expect(certificates[3]).equal("30-12-2024");
  });
});
...
describe("Fail Test", function () {
  ...
  it("Test for another account", async function () {
    ...
    await expect(
      cert.connect(other).issue(101, "Maneesh", "EDP", "M", "30-12-2024")
